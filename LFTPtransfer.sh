...
function TransferPayload()
{
    ...
    # Try to grab as a directory
        lftp -u ${CREDS} sftp://${HOST}/ -e "set xfer:use-temp-file yes; set xfer:temp-file-name '.lftp'; set net:limit-rate $RATE; $HOSTKEYFIX; mirror -c --parallel=$PARALLEL --use-pget-n=$SEGMENTS --exclude-glob \"*.nfo\" --exclude-glob \"*.srr\" --exclude-glob \"*.srt\" --exclude \"/$EXCLUDE_DIRS\" \"${_target}\" ;quit" >>/tmp/lftp$$.log 2>&1
